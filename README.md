...
- **A visual DB browser.** [`vim-dadbod-ui`](https://github.com/kristijanhusak/vim-dadbod-ui) (on [`vim-dadbod`](https://github.com/tpope/vim-dadbod)) — browse MongoDB collections in a tree, click one to see its documents, write and run raw `mongosh`-shell queries in the same buffer. Requires `mongosh` on PATH; connection comes from the `DBUI_URL`/`DBUI_NAME` environment variables, never committed here. Patches in the `List` query template for the `mongodb+srv` scheme (every Atlas connection string), which upstream only registers under plain `mongodb`, leaving `List` blank otherwise.
